...
\newcommand{\starsubsection}[1]{\addtocounter{subsection}{1}%
	\addcontentsline{toc}{section}{*\thesubsection\hspace{1em}#1}%
	{\vspace{1.5em}\par\noindent\bfseries*\thesubsection\hspace{0.5em} #1}\par\vspace{1em}\noindent}
...
\section{Метод Ритца}	
	...
	Каким же образом строится эта минимизирующая последовательность в методе Ритца?
	...
	\]
	принадлежит тому же классу $M$. Исходная задача заменяется следующей:
	\[
		\min_{c_1, \dots, c_n}V[c_1\varphi_1 + \dots + c_2\varphi_n]
	\]
...
\subsection{Кусочно-линейный базис}
	...
	интегрирования (например, можно использовать правило Симпсона, см. Приложение A).
	...
	\begin{Ex}
		...
		\[
			...
			\pi^2y^2(x) - 4\pi^2y(x)\sin{\pi x}\right)}dx\,,
		\]
	...
	Завершим рассмотрение метода Ритца, показав, каким образом к рассмотренной
	частной постановке можно свести задачу с неоднородными условиями и на произвольном
	сегменте. Пусть необходимо найти минимум функционала
	\[
		V[y] = \int\limits_a^b{\left[p(x)\left(y'(x)\right)^2 + q(x)y^2(x) - 2f(x)y(x)\right]}dx
	\]
	на множестве функций $M = \left\{y(x)\,\, |\,\, y(x) \in C^2_{[a,b]},\,\, y(a)=y(b)=0 \right\}$.
	Введем новую независимую переменную:
	\[
		\xi = \frac{x-a}{b-a} \quad \Rightarrow \quad \frac{d\xi}{dx} = \frac{1}{b-a} 
		\quad \Rightarrow \quad \frac{dy}{dx} = 
			\frac{dy}{d\xi}\cdot\frac{d\xi}{dx} =\frac{1}{b-a}y'(\xi) .
	\]
	Тогда функционал запишется в следующем виде:
	\begin{align*}
		V[y(\xi)]& = \int\limits_0^1\biggl[
				p((b-a)\xi+a)\left(\frac{y'(\xi)}{b-a}\right)^2 + q((b-a)\xi+a)y^2(\xi) - \\
			 & - 2f((b-a)\xi+a)y(\xi)\biggr](b-a)d\xi = \\
			 & =  \int\limits_0^1\left[
				P(\xi)\left(y'(\xi)\right)^2 + Q(\xi)y^2(\xi) - 2F(\xi)y(\xi)\right]d\xi .
	\end{align*}
	Получившуюся задачу можно решать, воспользовавшись описанными выше алгоритмами.

	Пусть теперь множество допустимых функций имеет вид
	\[
		M = \left\{y(x)\,\, |\,\, y(x) \in C^2_{[0,1]},\,\, y(0)=\alpha, y(1)=\beta \right\}.
	\]
	Чтобы избавиться от неоднородных краевых условий, представим решение в виде суммы 
	$y(x) = z(x)+h(x)$, где $z(x)$ удовлетворяет однородным условиям, а $h(x)$ --- неоднородным.
	Например, в качестве последней функции можно взять $h(x) = \beta x + (1-x)\alpha$.
	Можно показать, что при подстановке в функционал получим 
	\[
		V[y] = \int\limits_0^1{\left[p(x)\left(z'(x)\right)^2 + q(x)z^2(x) - 
		2\left(f(x)-q(x)h(x) - \left(p(x)h'(x)\right)'\right)z(x)\right]}dx + h_0\,,
	\]
	где $h_0=const$ зависит только от выбора $h(x)$. Таким образом, исходная задача свелась
	к эквивалентной ей однородной задаче относительно новой неизвестной функции $z(x)$.

\starsubsection{Базис из B-сплайнов}
	...
	\end{tikzpicture}
	\end{center}
	Разобьем промежуток $[0,1]$, задавшись натуральным $n$ и определив шаг 
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\draw (2.5cm, 2.5cm)  node {$y=\varphi_{n+1}(x)$};
		\end{axis}
	\end{tikzpicture}
	\\
	\end{center}
	...
	Значит, систему уравнений $Ac=b$ для неизвестных коэффициентов линейной аппроксимации
	...
	решенную при помощи кусочно-линейной аппроксимации.
	\begin{Ex}
